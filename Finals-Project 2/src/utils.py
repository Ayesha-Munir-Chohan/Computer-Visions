...
def classical_restore_rgb_from_gray(gray_input: np.ndarray, scratch_sensitivity: float = 0.48) -> np.ndarray:
    """
    Robust non-ML fallback for non-face images to avoid CNN artifacts.
    """
    gray_u8 = np.clip(gray_input * 255.0, 0, 255).astype(np.uint8)
    mask = build_scratch_mask(gray_input, sensitivity=scratch_sensitivity)
    clean = cv2.inpaint(gray_u8, mask, inpaintRadius=2, flags=cv2.INPAINT_TELEA)
    clean = cv2.fastNlMeansDenoising(clean, None, h=7, templateWindowSize=7, searchWindowSize=21)

    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    y = clahe.apply(clean)

    # Warm neutral tint for old-photo restoration look.
    r = np.clip(y.astype(np.float32) * 1.08, 0, 255).astype(np.uint8)
    g = np.clip(y.astype(np.float32) * 1.01, 0, 255).astype(np.uint8)
    b = np.clip(y.astype(np.float32) * 0.94, 0, 255).astype(np.uint8)
